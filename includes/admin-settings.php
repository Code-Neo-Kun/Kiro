...
function kiro_cursor_settings_page(): void
{
	...
	if (isset($_POST['kiro_cursor_save'])) {
		...
		$size = absint(wp_unslash($_POST['kiro_cursor_size'] ?? 12));
		...
		update_option(
			'kiro_cursor_style',
			kiro_cursor_sanitize_style(sanitize_text_field(wp_unslash($_POST['kiro_cursor_style'] ?? '')))
		);

		update_option(
			'kiro_cursor_animation',
			kiro_cursor_sanitize_animation(sanitize_text_field(wp_unslash($_POST['kiro_cursor_animation'] ?? '')))
		);
		...
		<form method="post" id="kiro-cursor-settings-form">
			...
			<div class="kiro-style-grid" id="kiro-style-grid">
				<?php foreach ($styles as $slug => $meta): ?>
					<label class="kiro-style-card <?php echo ($cursor_style === $slug) ? 'is-selected' : ''; ?>"
						...
						<input type="radio" name="kiro_cursor_style" value="<?php echo esc_attr($slug); ?>" <?php checked($cursor_style, $slug); ?> class="kiro-style-radio">
			...
			<div class="kiro-anim-grid" id="kiro-anim-grid">
				<?php foreach ($animations as $slug => $meta): ?>
					<label class="kiro-anim-card <?php echo ($animation === $slug) ? 'is-selected' : ''; ?>"
						...
						<input type="radio" name="kiro_cursor_animation" value="<?php echo esc_attr($slug); ?>" <?php checked($animation, $slug); ?> class="kiro-anim-radio">
			...
			<table class="form-table kiro-form-table">
				...
				<tr>
					...
					<td>
						<input type="color" id="kiro_cursor_color" name="kiro_cursor_color"
				...
				<tr>
					...
					<td>
						<input type="range" id="kiro_cursor_size" name="kiro_cursor_size"
							value="<?php echo esc_attr($size); ?>" min="4" max="60" step="1"
				...
				<tr>
					...
					<td>
						<label>
							<input type="checkbox" name="kiro_cursor_disable_mobile" value="1" <?php checked($mobile, 1); ?>>
			...
			<p class="submit">
				<input type="submit" name="kiro_cursor_save" class="button button-primary button-large"
